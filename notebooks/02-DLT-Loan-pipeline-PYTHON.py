...
# MAGIC select * from json.`/demo/danske-bank/dlt_loan/landing/accounts0.json`
...
def SV_cleaned_new_txs():
  ...
  return (
    txs.join(ref, F.col("txs.accounting_treatment_id") == F.col("ref.id"), "inner")
      .selectExpr("txs.*", "ref.id as accounting_treatment"))
...
def SV_historical_txs():
  return (
    dlt.read_stream("BZ_reference_loan_stats")
  )
